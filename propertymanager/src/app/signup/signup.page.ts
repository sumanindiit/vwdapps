...
export class SignupPage implements OnInit {
  ...
  onSubmit() {
    ...
    if (!this.signUpForm.valid) {
      ...
      formData.append('type', 'propertyManager');
      formData.append('role', 'propertyManager');
        ...
        .subscribe(
          (result) => {
            ...
            if (res.status == 422) {
              ...
              this.api.post('saveAdminNotifications', { type: 2, content: dict.email + ' has created an account as Property Manager' }, '')
                .subscribe(
                  (result) => { },
                  (error) => {
                    console.log(error);
