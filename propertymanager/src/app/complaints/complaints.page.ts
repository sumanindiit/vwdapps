...
export class ComplaintsPage implements OnInit {
  ...
  getPendingComplaints(event={},type='',searchItem = '' ){
    ...
    setTimeout(() => {
      self.api.post('getPropertyManagerComplaints', {userId:this.userId , recordsPerPage: self.recordsPerPage ,start: self.start ,status:1 ,community:this.userData.community,search: searchItem },'')
      .subscribe(
        (result) => {
  ...
  getCompletedComplaints(event={},type='',searchItem = '')
  {
    ...
    setTimeout(() => {
      self.api.post('getPropertyManagerComplaints', {userId:this.userId , recordsPerPage: self.recordsPerPage ,start: self.start ,status:3 ,community:this.userData.community ,search: searchItem},'')
      .subscribe(
        (result) => {
          this.common.stopLoading();
          const res: any = result;
